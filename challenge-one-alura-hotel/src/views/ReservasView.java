...
public class ReservasView extends JFrame {
	...
	//variable llamando a ReservaController
	...
	public ReservasView() {
		...
		//instanciamos reservaController que vamos a usar
       ...
       //Activa el evento, después del usuario seleccionar las fechas se debe calcular el valor de la reserva
			//usa el metodo calcularValor con las fechas elegidas...
		...
		btnsiguiente.addMouseListener(new MouseAdapter() {
			@Override
			public void mouseClicked(MouseEvent e) {
				//valida que las fechas de entrada y salida no sean nulos
				if (ReservasView.txtFechaE.getDate() != null && ReservasView.txtFechaS.getDate() != null &&  !ReservasView.txtValor.getText().trim().isEmpty()  ) {		
					//System.out.println(txtValor.getText());
					
					//llama a guaradr reserva
					guardarReserva();
	...
	//metodo para guardar reserva que va a ser llamado desde el btn guardar llamada del btnsigueinte
	public void guardarReserva() {
		//empezamos con try posibles errores en la ejecucion del proyecto
		try {
			//transformamos la fecha en string, la que traemos del dise;o creado q llamamos txtFechaE
			...
			//pasamos al constructor de reserva los 4 valores
			//la fecha la pasamos de string a DATE
			Reserva reserva = new Reserva(java.sql.Date.valueOf(fechaEntrada),
					java.sql.Date.valueOf(fechaSalida),
							txtValor.getText(), //txt traemos con getText
							txtFormaPago.getSelectedItem().toString());  //comboBox traemos el valor con SelectedItemm ... sekectedIndex trae no el valor sino la posicion del item en combobox
			
			
			//llamamos al metodo controllerReserva 
			reservaController.guardar(reserva);
			JOptionPane.showMessageDialog(contentPane, "Registro Creado Exitosamente");
			//mostramos un mje de guardado exitosamente
			
			//System.out.println(reserva.getId());

			//llama a la siguiente ventana de huesped
	...
	//calcularValor de hospedaje diario
	private void calcularValor(JDateChooser fechaE,JDateChooser fechaS) {
		//usa fecha de entrada y salida ... valida sino son nulas 
		if(fechaE.getDate()!= null && fechaS.getDate() !=null) {
			Calendar inicio = fechaE.getCalendar();
			Calendar fin = fechaS.getCalendar();
			//calcula dias 
			int dias = -1; // se usa -1 para contar a partir del dia siguiente
			int diaria = 180; //valor asignado de cuota dia de hospedaje
			int valor;
			Calendar hoy = Calendar.getInstance();
			
			
			if(fin.before(inicio)|| inicio.before(hoy) )
			{
				JOptionPane.showMessageDialog(contentPane, "Las fechas elegidas son incorrectas","Error", JOptionPane.ERROR_MESSAGE);
				txtValor.setText(""); 
			...
			//si fecha inicio anterior a fecha fin o es igual aumenta la cantidad de dias
			while(inicio.before(fin)|| inicio.equals(fin)) {
				dias++;
				inicio.add(Calendar.DATE,1); //cant dias a  ser aumentados
			}
			
			//calcula el valor que es la cantidad d dias d ehospedaje x 
			//valor diario de hospedaje
			valor = dias*diaria;
			txtValor.setText("" + valor); //seteamos el txtValor 
